...
class AppBar extends VertoBaseComponent {
  ...
  getDefaultStyle(styleName) {
    const styles = {
      ...
      appControlStyles: {
        ...
        '@media (max-width: 768px)': {
          // when less than 768px wide, hide this and show alt menu (hamburger)
          display: 'none',
        },
      },
      nsiCompStyle: {
      ...
      vcsCompStyle: {
        svgStyle: {
          marginRight: '20px',
          '@media (max-width: 768px)': {
            // when less than 768px wide, adjust for column orientation
            marginBottom: '10px',
            marginRight: '0px',
          },
        },
      ...
      altMenuStyles: {
        ...
        visbility: 'hidden',
        '@media (max-width: 768px)': {
          // when less than 768px wide, show this and hide app controls
          display: 'block',
          visibility: 'visible',
          cursor: 'pointer',
        },
  ...
  buildSettingsContainer() {
      ...
      netSpeedDisplay = (
        <div className="netSpeedContainer" style={{ ...this.getStyle('netSpeedContainer') }}>
          <span style={{ ...this.getStyle('outgoingSpacing') }}>
            <FormattedMessage id="BANDWIDTH_INFO_OUTGOING" defaultMessage="Outgoing:" />
          </span>
          <span style={{ ...this.getStyle('incomingSpacing') }}>
            <FormattedMessage id="BANDWIDTH_INFO_INCOMING" defaultMessage="Incoming:" />
          </span>
        </div>
      );
    } else {
    ...
    if (this.props.settings.useVideo === false) {
      useVideoAltDisplay = <div />;
    } else {
      useVideoAltDisplay = (
        <div>
          <SettingsMenuSelect
            cbSubmitSetting={(setting) => {
              this.props.dispatch(doUpdateSetting(setting));
            }}
      ...
      vidSettingsAltDisplay = (
        <div>
          ...
          <SettingsMenuSelect
            ...
            label={formatMessage({
              id: 'MAX_INCOMING_BANDWIDTH',
              defaultMessage: 'Max Incoming Bandwidth:',
            })}
            selectedOption={{
              id: 'incomingBandwidth',
              data: this.props.settings.incomingBandwidth,
            }}
          />
          <SettingsMenuSelect
            cbSubmitSetting={(setting) => {
            ...
            options={this.props.settings.bandwidth ? this.props.settings.bandwidth : []}
            label={formatMessage({
              id: 'MAX_OUTGOING_BANDWIDTH',
              defaultMessage: 'Max Outgoing Bandwidth:',
            })}
            selectedOption={{
              id: 'outgoingBandwidth',
              data: this.props.settings.outgoingBandwidth,
            }}
          />
        </div>
      );
      ...
      vidSettingsAltDisplay = (
        <div>
          <SettingsCheckbox
            ...
            label={formatMessage({
              id: 'RECHECK_BANDWIDTH',
              defaultMessage: 'Recheck Bandwidth Before Each Outgoing Call',
            })}
          ...
          <div className="buttonContainer" style={{ ...this.getStyle('buttonContainer') }}>
            <button
              ...
              onClick={() => {
                this.props.dispatch(doSpeedTest());
                this.showSpeeds();
              }}
            >
      ...
      >
        <div className="column1" style={{ ...this.getStyle('column1') }}>
          ...
          <SettingsMenuSelect
            ...
            selectedOption={{
              id: 'selectedBestFrameRate',
              data: this.props.settings.selectedBestFrameRate,
            }}
          />
          <div className="buttonContainer" style={{ ...this.getStyle('buttonContainer') }}>
            <button
            ...
            <span>
              <button
                ...
                onClick={() => {
                  this.props.dispatch(
                    doResolutionRefresh(false, this.props.app.settings.isRefreshing),
                  );
                }}
              >
                ...
                <div
                  style={
                    this.props.app.settings.isRefreshing
                      ? this.getStyle('loaderShowStyle')
                      : this.getStyle('loaderHiddenStyle')
                  }
                >
        ...
        <div className="column2" style={{ ...this.getStyle('column2') }}>
          ...
          <SettingsCheckbox
            ...
            label={formatMessage({
              id: 'SCALE_VIDEO',
              defaultMessage: 'Scale Remote Video To Match Camera Resolution',
            })}
          ...
          <SettingsCheckbox
            ...
            label={formatMessage({
              id: 'ASK_BEFORE_RECOVER',
              defaultMessage: 'Ask Before Recovering Call',
            })}
          ...
          <SettingsCheckbox
            ...
            checkedOption={{
              name: 'googEchoCancellation',
              value: this.props.settings.googEchoCancellation,
            }}
          />
          <SettingsCheckbox
            cbSubmitSetting={(setting) => {
            ...
            checkedOption={{
              name: 'googNoiseSuppression',
              value: this.props.settings.googNoiseSuppression,
            }}
          />
          <SettingsCheckbox
            cbSubmitSetting={(setting) => {
            ...
            checkedOption={{
              name: 'googHighpassFilter',
              value: this.props.settings.googHighpassFilter,
            }}
          />
        </div>
        <div className="column3" style={{ ...this.getStyle('column3') }}>
          <div style={{ ...this.getStyle('headerLabel') }}>
            <FormattedMessage id="VIDEO_SETTINGS" defaultMessage="Video settings:" />
          </div>
          <SettingsCheckbox
            cbSubmitSetting={(setting) => {
              this.props.dispatch(doUpdateSetting(setting));
            ...
            label={formatMessage({
              id: 'AUTO_SPEED_RES',
              defaultMessage: 'Automatically Determine Speed and Resolution Settings',
            })}
  ...
  render() {
    ...
    const loaderStyle = this.props.app.settings.isRefreshing
      ? this.getStyle('loaderShowStyle')
      : this.getStyle('loaderHiddenStyle');
    ...
    if (this.props.bandwidthInfo.outgoingBandwidth && this.props.bandwidthInfo.incomingBandwidth) {
      ...
      nsIndicator = (
        <NetworkStatusIndicator
          ...
          compStyle={
            this.state.showAltAppControls
              ? this.getStyle('altNsiCompStyle')
              : this.getStyle('nsiCompStyle')
          }
          networkData={{
            upkpbs: this.props.bandwidthInfo.outgoingBandwidth,
    ...
    if (true && !this.state.showAltAppControls) {
      if (this.props.callInfo.currentCallId) {
        lastCall = (
          <LastCall
            ttPosition={!this.state.showAltAppControls ? 'bottom' : 'right'}
            labelText={formatMessage({ id: 'IN_CALL', defaultMessage: 'In Call' })}
        ...
        lastCall = (
          <LastCall
            ttPosition={!this.state.showAltAppControls ? 'bottom' : 'right'}
            labelText={formatMessage({ id: 'LAST_CALL', defaultMessage: 'Last Call:     ' })}
            lastNumber={this.props.callInfo.lastNumber}
            cbClick={(number) => {
        ...
        lastCall = (
          <LastCall
            ttPosition={!this.state.showAltAppControls ? 'bottom' : 'right'}
            labelText={formatMessage({ id: 'NO_CALL', defaultMessage: 'No Call' })}
          />
        );
      }
    }
    ...
    return (
      <div
        className="appbar"
        style={this.getStyle('appbarStyles')}
        onClick={(event) => {
      ...
      >
        <span
          className="altMenu"
          style={this.getStyle('altMenuStyles')}
        ...
        <span className="appControls" style={acStyles}>
          ...
          <VCStatus
            ...
            compStyle={
              !this.state.showAltAppControls
                ? { svgStyle: { marginRight: '20px' } }
                : { svgStyle: { marginBottom: '10px' } }
            }
          />
          {lastCall}
          ...
          <div
            style={
              !this.state.showAltAppControls
                ? this.getStyle('marginRightStyle')
                : this.getStyle('marginBottomStyle')
            }
          >
            <Settings
              ref="MySettingsMenu"
              allowDisplayDetails={this.props.vcStatus != 'disconnected'}
              cbSubmitSetting={(data) => {
                this.props.dispatch(doUpdateSetting(data));
              }}
              ...
              cbPreviewSet={() => {
                App.toggleModal(
                  <SettingsPreview settings={this.props.settings} cbClose={App.toggleModal} />,
                );
              }}
              ttPosition={!this.state.showAltAppControls ? 'bottom' : 'right'}
              status={this.props.vcStatus}
          ...
          >
            ...
            >
              <MenuItem
                label={<FormattedMessage id="OPEN_NEW_WINDOW" />}
                cbAction={() => {
            ...
            >
              <MenuItem
                ...
                cbAction={() => {
                  ...
                  App.toggleModal(
                    <About version="0.2.0" gitRev="xxxxx" cbClose={App.toggleModal} />,
                  );
                }}
              />
              <MenuItem
                label={formatMessage({ id: 'VIEW_ALERT', defaultMessage: 'Alert Log' })}
                cbAction={() => {
                  // TODO ta need to pass version and gitRev in to the About component
              ...
              />
              <MenuItem
                label={formatMessage({ id: 'TITLE_CONTRIBUTORS', defaultMessage: 'Contributors' })}
                cbAction={() => {
                  App.toggleModal(<Contributors contributorsData={this.props.contributorsData} />);
                }}
              />
              <MenuItem
                label={formatMessage({ id: 'HELP', defaultMessage: 'Help' })}
                cbAction={() => {
                  window.open(
